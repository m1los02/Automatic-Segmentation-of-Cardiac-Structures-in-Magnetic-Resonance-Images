...
import glob
from utils import TARGET_SPACING

def resample_itk(img: sitk.Image, spacing=TARGET_SPACING, is_label=False):
    """Resample image to given spacing (sx, sy, sz)"""
    orig_sp = np.array(img.GetSpacing(), dtype=np.float32)
    orig_sz = np.array(img.GetSize(),    dtype=np.int64)
    new_sp  = np.array(spacing, dtype=np.float32)
    new_sz  = np.round(orig_sz * (orig_sp / new_sp)).astype(np.int64).tolist()

    res = sitk.ResampleImageFilter()
    res.SetOutputSpacing(tuple(map(float, new_sp)))
    res.SetSize([int(s) for s in new_sz])
...
def resize_segmentation(segmentation, new_shape, order=0, cval=0):
    '''
    Resizes a segmentation map. Will transform segmentation map to one
...
def resample(img,old_spacing,new_spacing=(1.0,1.0,1.0),data_type='img'):
    '''
    Rasample from old to new spacing
    '''
    shape = np.shape(img)
...
if __name__ == '__main__':
    ...
    for pid in patient_list:
        ...
        #resample and crop to 192x192(224x224)
